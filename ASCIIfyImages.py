...
img_array = os.listdir("./Frames")
if not os.path.exists('ASCIIFrames'):
    os.makedirs('ASCIIFrames')

k=0
while(k<len(img_array)):
    # Reading Input Image
    currpath="./Frames/frame"+str(k)+".jpg"
    ...
    filename="./ASCIIFrames/ASCIIframe"+str(k)+".jpg"
    out_image.save(filename)
    k=k+1
    print("ASCIIfyed frame "+str(k)+" of "+str(len(img_array)))
